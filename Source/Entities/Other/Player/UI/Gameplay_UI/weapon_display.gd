...
func _ready():
	visible = false
	$MarginContainer/VBoxContainer/WeaponSprite.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
...
func _on_weapon_changed(weapon):
	...
	if weapon == null:
		...
		visible = true
		$MarginContainer/VBoxContainer/WeaponSprite.texture = weapon.sprite

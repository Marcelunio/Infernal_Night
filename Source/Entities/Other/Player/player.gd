...
func _handle_player_movement():#obsluguje ruch gracza
	...
	_handle_move_recoil(input_dir)
	
	
	move_and_slide()

func _handle_move_recoil(input_dir: Vector2):
	var weapon = inventory.current_weapon
	var move_multiplier := 1.0

	if weapon and weapon.is_in_group("weapon-heavy-mg") and weapon.shooting:
		if input_dir != Vector2.ZERO:
			move_multiplier = 0.4
		else:
			var recoil_dir = -weapon.shoot_direction.normalized()
			recoil_velocity += recoil_dir * weapon.recoil_force * delta

	var desired_velocity = input_dir * speed * move_multiplier
